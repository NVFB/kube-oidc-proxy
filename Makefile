...
build: generate ## build kube-oidc-proxy
	mkdir -p ./bin/amd64/{linux,darwin}
	mkdir -p ./bin/arm64/{linux,darwin}
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags '-w $(shell hack/version-ldflags.sh)' -o ./bin/amd64/linux/kube-oidc-proxy ./cmd/.
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags '-w $(shell hack/version-ldflags.sh)' -o ./bin/arm64/linux/kube-oidc-proxy ./cmd/.
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags '-w $(shell hack/version-ldflags.sh)' -o ./bin/amd64/darwin/kube-oidc-proxy ./cmd/.
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags '-w $(shell hack/version-ldflags.sh)' -o ./bin/arm64/darwin/kube-oidc-proxy ./cmd/.
